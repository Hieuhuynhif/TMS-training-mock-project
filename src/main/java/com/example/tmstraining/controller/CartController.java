...
public class CartController {
    ...
    @PostMapping("")
    public Cart addItemToCart(@AuthenticationPrincipal CustomUserDetails userDetails, @RequestBody ItemCartRequest item) {
        int userId = userDetails.getId();
        return cartService.addItemToCart(userId, item);
    }

    @PutMapping("")
    public ResponseEntity<String> updateCart(@RequestBody ItemCartRequest itemCartRequest, @AuthenticationPrincipal CustomUserDetails userDetails) {
        int userId = userDetails.getId();
        return cartService.updateItemInCart(userId, itemCartRequest);
    }

    @DeleteMapping("{cartDetailsId}")
    public ResponseEntity<String> deleteCartDetails(@PathVariable int cartDetailsId, @AuthenticationPrincipal CustomUserDetails userDetails) {
        int userId = userDetails.getId();
